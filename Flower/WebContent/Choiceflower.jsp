	...
	<%int a = productList.get(i).getProduct_store(); %>
		...
		<tr>
			<td width = "500px" height = 450px >			
				<a href = "Order.jsp?number=<%= productList.get(i).getProduct_number()%>&listNum=<%=i%>">
				<img width = "400px" height = auto src = <%= productList.get(i).getProduct_image()%>></a>
			<td>
			<td width = "300px">
				...
				<%for (int j = 0; j <= 11; j++) {%>
					<% if (a == j+1){ %>
						...
						매장전화 : <%= storeList.get(j).getStore_tel()%>
					<%} %>
				<%} %>
				<br><br>
